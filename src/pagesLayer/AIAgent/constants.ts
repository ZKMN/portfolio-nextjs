...
export const TOOL_CATEGORIES: ToolCategory[] = [
  {
    ...
    color: 'var(--accent)',
    tools: [
      { name: 'Studio Finder', description: '7-priority fuzzy search chain (handles misspellings, districts, cross-locale)' },
      { name: 'Studio Details', description: 'Address, hours, phone, Google Maps link, video directions' },
      { name: 'Country List', description: 'Countries with active studios' },
    ],
  },
  {
    ...
    color: 'var(--accent-2)',
    tools: [
      { name: 'Service Browser', description: 'Zones (legs, face, bikini) or procedures (cosmetology)' },
      { name: 'Price Lookup', description: 'Structured price overview with device comparison support' },
      { name: 'Subscription Browser', description: 'Subscription packages with calculated savings' },
      { name: 'Device Info', description: 'Full device specs, comparisons, contraindications' },
      { name: 'Upsell Prompt', description: '"Want to add more zones?" with yes/no selector' },
    ],
  },
  {
    name: 'Cart & Booking',
    color: 'var(--success)',
    tools: [
      { name: 'Add to Cart', description: 'Validates serviceId against lastShownServices' },
      { name: 'Remove from Cart', description: 'Clear specific item from cart' },
      { name: 'Availability Check', description: 'Available time slots for a specific date' },
      { name: 'Booking Link', description: 'Generates booking URL, validates time against availability' },
      { name: 'Repeat Booking', description: 'Re-book from permanent booking history' },
    ],
  },
  {
    ...
    color: '#f59e0b',
    tools: [
      { name: 'Knowledge Search', description: 'Two-stage semantic search: context-first, then global fallback' },
      { name: 'Human Handoff', description: 'Escalates to human support, creates incident snapshot' },
      { name: 'Consultation Offer', description: 'Proactive consultation offer before/after procedure selection' },
    ],
  },
  {
    name: 'Flow Control',
    color: '#ec4899',
    tools: [
      { name: 'Gender Selector', description: 'Gender selection with UI selector when unset' },
      { name: 'Category Picker', description: 'Category selection with automatic device inference' },
      { name: 'Device Resolver', description: 'Device inference from category via DEVICE_TO_CATEGORY mapping' },
      { name: 'Cart Overview', description: 'Display current cart with totals' },
      { name: 'Session Reset', description: 'Clear session after completing booking' },
    ],
  },
];
